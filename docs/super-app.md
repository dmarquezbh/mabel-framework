...
> framework de UI cross-platform, é a **plataforma de super-app poliglota da Org**.
...
Um único app **Org** na loja/no device. As funcionalidades **não** são telas
...
"como fazer um app cross-platform" para "**como a Org entrega software no device**".

### Pilares

- **Um app, muitas features.** Org é o app; Kanban, Chat, e o que cada time construir
  são mini-apps dentro dele. Crescem sem passar de novo pela loja (ver `docs/ota.md`).
- **Poliglota por time.** Cada time no seu stack — Ledger em .NET/Blazor, outro em
...
estritamente melhor pro caso Org:
...
```
  ...
  │   │ Kanban (SDUI) │  │ Chat (webview│  │ time-X (SDUI)│  …     │
...
Mini-apps não se enxergam diretamente (sandbox). Se o Kanban precisa abrir o Chat num
...
> **Publicar ou atualizar o mini-app Ledger NÃO quebra o Chat** (nem qualquer outro
...
>    atualizar o Ledger não toca o binário/descritor do Chat.
>
...
## 4. Incorporando o Chat (caminho real)

O Chat já é web (`<rui-assessor>`). Dois caminhos, e o modelo **misto** é suportado:

1. **Rápido — mini-app webview:** o Chat entra como um mini-app cujo "render" é uma
   `WKWebView` (iOS) / `WebView2`/WebKitGTK (desktop) hospedada pelo shell, ao lado dos
   mini-apps SDUI-nativos. Reusa a web Chat **hoje**, com auth/capabilities/navegação do
   shell. É a ponte pragmática.
2. **Depois — migra pra SDUI-nativo:** reescreve a UI do Chat como emissor de descritor
...
- **Baked (AOT no build):** os mini-apps conhecidos entram compilados no binário Org.
...
(guideline 2.5.2). Para a Org isso **não é bloqueio**:
...
- **É:** o modelo de plataforma (shell multi-módulo, mini-apps sandbox, serviços
  compartilhados, registry, incorporação do Chat, framing organizacional Org).
...
3. **Chat: webview-mini-app já ou espera SDUI?** — recomendo webview-mini-app primeiro
