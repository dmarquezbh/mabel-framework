...
- **Pedido:** Daniel (Tech Lead), verbal — avaliar o Mabel ficar "UI agnostic" e permitir
  ...
  a partir do app nativo (Rui Native), rodando no device físico **crow5**.
...
`ios_app` gerado, e não tentei nenhum deploy contra o crow5 para a parte Flutter.
...
O restante do pipeline nativo (build/bundle/codesign/install no crow5, sem
...
- Device físico **crow5** (iPhone XS Max, iOS 18.7.9) conectado via USB e
...
2. **Bloqueador já conhecido e documentado (não é novo, não tentei resolver de
   ...
   integração de frameworks funcionando, o **install final no crow5** passaria
...
| Validação ponta a ponta real no crow5 (depende do bloqueio de provisioning profile estar resolvido) | 0.5-1 dia, **mas gated** | Não dá pra estimar com confiança até o bloqueio de profile ser resolvido — pode revelar mais problemas de assinatura/rpath só visíveis em device real. |
