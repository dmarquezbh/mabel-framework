...
// =============================================================================
// Entry point do host Windows. Le o descritor SDUI (board-sdui.json emitido
// pelo board_gen), constroi a arvore de controles NATIVOS WPF e abre a janela.
//
//   Mabel.Host.Windows.exe            -> abre a janela do Board (GUI).
...
public static class Program
{
    ...
    public static int Main(string[] args)
    {
        ...
        if (jsonPath is null)
        {
            Console.Error.WriteLine("board-sdui.json nao encontrado (assets/ ao lado do exe).");
            return 2;
        }
        ...
        var win = new Window
        {
            Title = "Mabel Host Windows - Board (SDUI nativo)",
            Width = 1200,
            Height = 820,
            Content = layout,
    ...
    private static string? LocateDescriptor(string[] args)
    {
        ...
        var candidates = new[]
        {
            Path.Combine(AppContext.BaseDirectory, "assets", "board-sdui.json"),
            Path.Combine(AppContext.BaseDirectory, "board-sdui.json"),
        };
