...
## O panorama: a plataforma de super-app da PJUS

> **Cada projeto/time da PJUS entrega o seu mini-app; o super-app PJUS renderiza todos.**

Há **um único app PJUS** no device. As features não são telas hard-coded — são **mini-apps**
...
- **Um app, muitas features.** PJUS é o app; Board, Aria e o que cada time construir são
  mini-apps dentro dele. Crescem sem passar de novo pela loja.
- **Poliglota por time.** Cada time no seu stack — Opera em .NET/Blazor, outro em Go/Rust —
...
flowchart TD
    ...
    subgraph shell["super-app PJUS = shell nativo fino (o host)"]
...
Os dois caminhos iOS rodaram **lado a lado no mesmo app no iPhone crow5**, cross-compilados e
...
interpretado = cinza-público. **Interno PJUS não tem esse limite.** E o **descritor-OTA**
...
**Policy, honesto:** PJUS **enterprise/interno/MDM** = OTA livre (não passa por App Review).
...
Opera, melhor DX); Go/Rust habilitados publicando o WIT + o gerador. A arquitetura
...
logando `[Board] open-card card:X` em colunas diferentes (a prova do Board, confirmada pelo Daniel).
...
- **Descritor SDUI → UIKit nativo num iPhone físico** (ADR 0001): card-flash + scroll nativo
  + 5 taps logando `[Board] open-card card:X` em colunas diferentes (confirmado pelo Daniel).
